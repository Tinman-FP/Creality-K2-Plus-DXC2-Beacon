...
class BeaconProbe:
    def __init__(self, config, sensor_id):
        ...
        # The K2 Plus reaches USB Beacon through a userspace USB-to-PTY
        # bridge because its vendor kernel does not provide cdc_acm.  That
        # extra scheduling hop can exceed Beacon's 25 ms multi-MCU trsync
        # window even while the serial link is healthy.  Current upstream
        # Beacon exposes this setting and permits values up to 250 ms; carry
        # that option into the Plus 4-compatible driver used here.
        self.trsync_timeout = config.getfloat(
            "trsync_timeout", TRSYNC_TIMEOUT, above=0.0, maxval=0.25
        )
        ...
        # Beacon's full-rate stream can saturate the K2 Plus host CPU when it
        # calculates a trapq position for every sample.  Keep a configurable
        # fraction of streamed samples; firmware-side contact and homing
        # detection remain full rate.  1/8 still provides hundreds of host
        # samples per second for a 200 mm/s scan.
        self.stream_sample_decimation = config.getint(
            "stream_sample_decimation", 1, minval=1, maxval=64
        )
        self._stream_decimation_counter = 0
        self.trapq = None
    ...
    def _build_config(self):
        ...
        try:
            ...
            if (
                constants.get("BEACON_HAS_ACCEL", 0) == 1
                and self.accel_config.enabled
            ):
    ...
    def _handle_beacon_data(self, params):
        ...
        if self._stream_en and self.stream_sample_decimation > 1:
            kept = []
            for sample in samples:
                if self._stream_decimation_counter == 0:
                    kept.append(sample)
                self._stream_decimation_counter = (
                    self._stream_decimation_counter + 1
                ) % self.stream_sample_decimation
            samples = kept
            if not samples:
                return
    ...
    def _apply_mesh(self, matrix, gcmd):
        ...
        try:
            ...
            try:
                ...
                # Creality's K2 fork passes the printer object to ZMesh so it
                # can allocate its native UI/shared-memory mesh structures.
                mesh = bed_mesh.ZMesh(params, self.beacon.printer)
        try:
...
class BeaconAccelConfig(object):
    def __init__(self, config):
        self.enabled = config.getboolean("accel_enable", True)
