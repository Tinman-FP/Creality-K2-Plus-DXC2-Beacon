    ...
    def homing_move(self, movepos, speed, probe_pos=False,
        ...
        for mcu_endstop, name in self.endstops:
            ...
            if trigger_time > 0.:
                ...
                if name == "z" and self.prtouch_v3 is not None:
                    error = None
    ...
    def home_rails(self, rails, forcepos, movepos):
        ...
        if hi.retract_dist:
            ...
            if (hmove.check_no_movement() is not None
                    and rails[0].get_name() == "stepper_z"
                    and hmove.prtouch_v3 is not None):
            ...
            if self.probe_type == "prtouch_v2":
                ...
                for a in axes:
                    if a == 0 or a == 1:
                        ...
                        # Beacon's proximity homing has already synchronized Z
                        # with its own virtual endstop.  Do not send that one
                        # synthetic G28 Z through Creality's bottom optical
                        # Z-align sequence.
                        if gcmd.get("BEACON", None) is not None:
                            kin.home(homing_state)
                        elif self.config.has_section("z_align"):
