...
| K2 reports cutter contact/return OK but filament remains continuous or leaves a tail | Contact feedback confirms actuator/X contact, not filament severance; cutter travel may be insufficient | Do not tune `Tn_retrude` first. Verify cutter mechanics, lower `cut_pos_offset` in 0.1 mm steps, restart, recalibrate, and inspect the cut after every test. The reference machine is currently at `0.1` |
| Direct `BOX_LOAD_MATERIAL TNN=...` shuts Klipper down with `KeyError: None` | The outer Creality macro drops `TNN` before the nested feed/flush steps | Merge the optional parameter-forwarding change in `config/dxc2/box-load-tnn-forward.cfg.example`; restart and test with a watched load |
...
| Extruder board LED is on but `0x85` never replies | Power is present, but the controller data path is open or the controller is not communicating | The LED is not a communication test. Try a known-good motor/controller on the same cable without commanding motion. If it is also silent, test or replace the short motor cable, then the toolboard, then the main toolhead harness |
| Original and known-good extruder motors are both silent on the same cable | Failure is upstream of the motor assembly | Do not buy another motor. Inspect both cable ends and latches; substitute the short motor cable first, then the toolboard. Escalate to the main toolhead cable only if the first two do not restore `0x85` |
...
| Beacon is visible in `lsusb`, bridge process says running, but `/dev/cartographer` is absent after boot | Userspace bridge startup race left a stale process without its PTY link | Restart `/etc/init.d/beacon`, verify `/dev/cartographer`, then restart `/etc/init.d/klipper`. Do not repeatedly restart Klipper while the device link is absent |
